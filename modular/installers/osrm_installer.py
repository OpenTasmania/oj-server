...
class OsrmInstaller(BaseInstaller):
    ...
    def install(self) -> bool:
        ...
        try:
            ...
            log_map_server(
                f"{config.SYMBOLS['info']} Installing OSRM and its dependencies...",
                "info",
                self.logger,
            )

            # Ensure OSRM dependencies
            if not self._ensure_osrm_dependencies():
                return False
            ...
            log_map_server(
                f"{config.SYMBOLS['success']} OSRM and its dependencies installed successfully.",
                "success",
                self.logger,
            )
    ...
    def uninstall(self) -> bool:
        """
        Uninstall OSRM and its dependencies.
    ...
    def is_installed(self) -> bool:
        ...
        """
        # Check if OSRM dependencies are installed
        packages = ["osmium-tool", "osmctools", "jq"]
        for pkg in packages:
    ...
    def _ensure_osrm_dependencies(self) -> bool:
        """
        Ensure that OSRM dependencies are installed.
        ...
        try:
            log_map_server(
                f"{config.SYMBOLS['info']} Ensuring OSRM dependencies are installed...",
                "info",
                self.logger,
            )
            ...
            if not all_installed:
                log_map_server(
                    f"{config.SYMBOLS['error']} Failed to install all required OSRM dependencies.",
                    "error",
                    self.logger,
                )
                return False
            ...
            log_map_server(
                f"{config.SYMBOLS['success']} All OSRM dependencies are installed.",
                "success",
                self.logger,
            )
            ...
            log_map_server(
                f"{config.SYMBOLS['error']} Error ensuring OSRM dependencies: {str(e)}",
                "error",
                self.logger,
            )
            return False
