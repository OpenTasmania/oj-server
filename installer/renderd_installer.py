...
RENDERD_SYSTEM_USER = "www-data"  # User renderd runs as (often www-data for Apache integration)
...
RENDERD_CONF_FILE_SYSTEM_PATH = "/etc/renderd.conf"  # Standard path for renderd.conf
...
def ensure_renderd_packages_installed(
    ...
    log_map_server(f"{symbols.get('info', 'ℹ️')} Checking Renderd package installation status...", "info",
    ...
    for pkg in packages_to_check:
        if check_package_installed(pkg, app_settings=app_settings, current_logger=logger_to_use):
            log_map_server(f"{symbols.get('success', '✅')} Package '{pkg}' is installed.", "debug", logger_to_use,
                           app_settings)
        else:
            log_map_server(
    ...
    log_map_server(f"{symbols.get('success', '✅')} Renderd related packages confirmed.", "success", logger_to_use,
                   app_settings)
...
def create_renderd_directories(
    ...
    logger_to_use = current_logger if current_logger else module_logger
    symbols = app_settings.symbols
    log_map_server(f"{symbols.get('step', '➡️')} Creating directories for Renderd...", "info", logger_to_use,
                   app_settings)

    # Get paths from app_settings.renderd
    tile_cache_dir = str(app_settings.renderd.tile_cache_dir)  # Ensure string for commands
    ...
    for dir_path, (owner, group) in dirs_to_create_and_own.items():
        run_elevated_command(["mkdir", "-p", dir_path], app_settings, current_logger=logger_to_use)
        run_elevated_command(["chown", "-R", f"{owner}:{group}", dir_path], app_settings, current_logger=logger_to_use)
        run_elevated_command(["chmod", "-R", "u+rwX,g+rX,o+rX", dir_path], app_settings, current_logger=logger_to_use)
        log_map_server(
...
def create_renderd_systemd_service_file(
    ...
    log_map_server(f"{symbols.get('step', '➡️')} Creating Renderd systemd service file...", "info", logger_to_use,
                   app_settings)

    renderd_service_path = "/etc/systemd/system/renderd.service"  # Standard system path
...
"""
    ...
    try:
        run_elevated_command(
            ["tee", renderd_service_path], app_settings,
            cmd_input=final_service_content, current_logger=logger_to_use
        )
        log_map_server(f"{symbols.get('success', '✅')} Created/Updated {renderd_service_path}", "success",
                       logger_to_use, app_settings)
    except Exception as e:
        log_map_server(f"{symbols.get('error', '❌')} Failed to write {renderd_service_path}: {e}", "error",
                       logger_to_use, app_settings)
        raise