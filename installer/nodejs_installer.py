...
def install_nodejs_lts(
    ...
    log_map_server(
        f"{symbols.get('step', '➡️')} Installing Node.js LTS using NodeSource...",
        "info",
        logger_to_use,
        app_settings,
    )
    try:
        ...
        log_map_server(
            f"{symbols.get('gear', '⚙️')} Downloading NodeSource script from {nodesource_setup_url}...",
            "info",
            logger_to_use,
            app_settings,
        )
        ...
        log_map_server(
            f"{symbols.get('gear', '⚙️')} Executing NodeSource setup script...",
            "info",
            logger_to_use,
            app_settings,
        )
        ...
        log_map_server(
            f"{symbols.get('package', '📦')} Installing Node.js...",
            "info",
            logger_to_use,
            app_settings,
        )
        ...
        log_map_server(
            f"{symbols.get('success', '✅')} Node.js installed. Version: {node_ver}, NPM Version: {npm_ver}",
            "success",
            logger_to_use,
            app_settings,
        )
        ...
        log_map_server(
            f"{symbols.get('error', '❌')} Failed to install Node.js LTS: {e}",
            "error",
            logger_to_use,
            app_settings,
            exc_info=True,
