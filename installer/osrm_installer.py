...
def ensure_osrm_dependencies(app_settings: AppSettings, current_logger: Optional[logging.Logger] = None) -> None:
    ...
    log_map_server(f"{symbols.get('info', 'ℹ️')} Checking OSRM dependencies ({container_cmd}, osmium-tool)...", "info",
    ...
    log_map_server(f"{symbols.get('success', '✅')} Container runtime '{container_cmd}' is available.", "success",
                   logger_to_use, app_settings)

    if not check_package_installed("osmium-tool", app_settings=app_settings, current_logger=logger_to_use):
        log_map_server(f"{symbols.get('error', '❌')} 'osmium-tool' not installed. Required for PBF extraction.",
                       "error", logger_to_use, app_settings)
        raise EnvironmentError("'osmium-tool' not found. Ensure mapping packages are installed.")
    log_map_server(f"{symbols.get('success', '✅')} 'osmium-tool' is available.", "success", logger_to_use, app_settings)

    if not command_exists("wget"):
        log_map_server(f"{symbols.get('error', '❌')} 'wget' not installed. Required for PBF download.", "error",
                       logger_to_use, app_settings)
        raise EnvironmentError("'wget' not found.")
    log_map_server(f"{symbols.get('success', '✅')} 'wget' is available.", "success", logger_to_use, app_settings)


def setup_osrm_data_directories(app_settings: AppSettings, current_logger: Optional[logging.Logger] = None) -> None:
    """Creates base directories for OSRM source data and processed files from app_settings."""
    logger_to_use = current_logger if current_logger else module_logger
    symbols = app_settings.symbols
    osrm_data_cfg = app_settings.osrm_data

    log_map_server(f"{symbols.get('step', '➡️')} Setting up OSRM data directories...", "info", logger_to_use,
                   app_settings)
    ...
    osm_data_regions_dir = str(Path(osm_data_base_dir) / "regions")  # Standardized sub-path
    ...
    for dir_path_str in dirs_to_create:
        ...
        if not dir_path.exists():
            run_elevated_command(["mkdir", "-p", str(dir_path)], app_settings, current_logger=logger_to_use)
            log_map_server(f"{symbols.get('success', '✅')} Created directory: {dir_path}", "info", logger_to_use,
                           app_settings)
        ...
        run_elevated_command(["chown", f"{current_uid_str}:{current_gid_str}", str(dir_path)], app_settings,
                             current_logger=logger_to_use)
        run_elevated_command(["chmod", "u+rwx,g+rx,o+rx", str(dir_path)], app_settings,
                             current_logger=logger_to_use)  # 755 effectively for user focus
        log_map_server(f"{symbols.get('info', 'ℹ️')} Ensured ownership and permissions for directory: {dir_path}",
                       "info", logger_to_use, app_settings)


def download_base_pbf(app_settings: AppSettings, current_logger: Optional[logging.Logger] = None) -> str:
    ...
    log_map_server(f"{symbols.get('step', '➡️')} Managing base PBF file: {pbf_filename}...", "info", logger_to_use,
                   app_settings)
    ...
    if not os.path.isfile(pbf_full_path):
        ...
        log_map_server(f"{symbols.get('success', '✅')} Downloaded {pbf_filename}.", "success", logger_to_use,
                       app_settings)
    else:
        log_map_server(f"{symbols.get('info', 'ℹ️')} Base PBF file {pbf_full_path} already exists. Skipping download.",
                       "info", logger_to_use, app_settings)
...
def prepare_region_boundaries(app_settings: AppSettings, current_logger: Optional[logging.Logger] = None) -> None:
    """Copies GeoJSON region boundary files from project assets to OSRM data regions directory."""
    logger_to_use = current_logger if current_logger else module_logger
    symbols = app_settings.symbols
    osrm_data_cfg = app_settings.osrm_data

    log_map_server(f"{symbols.get('step', '➡️')} Preparing region boundary files (GeoJSON)...", "info", logger_to_use,
                   app_settings)

    # OSM_PROJECT_ROOT comes from static_config
    assets_source_regions_dir = static_config.OSM_PROJECT_ROOT / "assets" / "regions"
    ...
    for root, _, files in os.walk(assets_source_regions_dir):
        ...
        if not target_current_dir.exists():  # Create subdirectories in target if they don't exist
            target_current_dir.mkdir(parents=True, exist_ok=True)
            # Ownership should be fine due to parent dir permissions.

        for file_name in files:
            if file_name.endswith(".json"):  # Assuming GeoJSON files end with .json
                ...
                try:
                    ...
                    log_map_server(f"Copied boundary file: {target_file}", "debug", logger_to_use, app_settings)
                    copied_files_count += 1
                except Exception as e_copy:
                    log_map_server(
                        f"{symbols.get('error', '❌')} Failed to copy {source_file} to {target_file}: {e_copy}", "error",
                        logger_to_use, app_settings)
    ...
    if copied_files_count > 0:
        ...
        log_map_server(f"{symbols.get('warning', '!')} No boundary files were copied. Check assets directory.",
                       "warning", logger_to_use, app_settings)