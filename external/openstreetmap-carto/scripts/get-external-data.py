...
def database_setup(conn, temp_schema, schema, metadata_table):
    with conn.cursor() as cur:
        ...
        cur.execute(sql.SQL('''CREATE SCHEMA IF NOT EXISTS {temp_schema};''')
                    .format(temp_schema=sql.Identifier(temp_schema)))
        cur.execute(sql.SQL('''CREATE TABLE IF NOT EXISTS {schema}.{metadata_table}
                               (
                                   name
                                   text
                                   primary
...
class Table:
    ...
    def clean_temp(self):
        with self._conn.cursor() as cur:
            cur.execute(sql.SQL('''DROP TABLE IF EXISTS {temp_schema}.{name}''')
                        .format(name=sql.Identifier(self._name),
    ...
    def last_modified(self):
        with self._conn.cursor() as cur:
            cur.execute(sql.SQL('''SELECT last_modified
                                   FROM {schema}.{metadata_table}
                                   WHERE name = %s''')
                        .format(schema=sql.Identifier(self._dst_schema),
    ...
    def grant_access(self, user_role):  # parameter renamed for clarity
        with self._conn.cursor() as cur:
            cur.execute(sql.SQL('''GRANT SELECT ON {temp_schema}.{name} TO {role};''')
                        .format(name=sql.Identifier(self._name),
                                temp_schema=sql.Identifier(self._temp_schema),
    ...
    def index(self):
        with self._conn.cursor() as cur:
            ...
            cur.execute(sql.SQL('''ALTER TABLE {temp_schema}.{name} SET ( autovacuum_enabled = FALSE );''')
                        .format(name=sql.Identifier(self._name), temp_schema=sql.Identifier(self._temp_schema)))
            ...
            try:
                cur.execute(sql.SQL('''ALTER TABLE {temp_schema}.{name} DROP COLUMN ogc_fid;''')
                            .format(name=sql.Identifier(self._name), temp_schema=sql.Identifier(self._temp_schema)))
            except psycopg.errors.UndefinedColumn:
                logging.warning(f"Column ogc_fid not found on table {self._temp_schema}.{self._name}, skipping drop.")

            # Null geometries are useless for rendering
            cur.execute(sql.SQL('''DELETE
                                   FROM {temp_schema}.{name}
                                   WHERE way IS NULL;''')
                        .format(name=sql.Identifier(self._name), temp_schema=sql.Identifier(self._temp_schema)))
            cur.execute(sql.SQL('''ALTER TABLE {temp_schema}.{name} ALTER COLUMN way SET NOT NULL;''')
                        .format(name=sql.Identifier(self._name), temp_schema=sql.Identifier(self._temp_schema)))

            # sorting static tables helps performance and reduces size from the column drop above
            idx_name = self._name + "_order"

            cur.execute(sql.SQL('''CREATE INDEX {index_identifier} ON {temp_schema}.{name} (ST_Envelope(way));''')
                        .format(index_identifier=sql.Identifier(idx_name),
                                name=sql.Identifier(self._name),
                                temp_schema=sql.Identifier(self._temp_schema)))

            cur.execute(sql.SQL('''CLUSTER {temp_schema}.{name} USING {index_identifier};''')
                        .format(name=sql.Identifier(self._name),
                                temp_schema=sql.Identifier(self._temp_schema),
                                index_identifier=sql.Identifier(idx_name)))  # CLUSTER uses unquoted index name

            # The index is created within the temp_schema, so it should be dropped from there.
            cur.execute(sql.SQL('''DROP INDEX {temp_schema}.{index_identifier};''')
                        .format(temp_schema=sql.Identifier(self._temp_schema),
                                index_identifier=sql.Identifier(idx_name)))

            cur.execute(sql.SQL('''CREATE INDEX ON {temp_schema}.{name} USING GIST (way) WITH (fillfactor=100);''')
                        .format(name=sql.Identifier(self._name), temp_schema=sql.Identifier(self._temp_schema)))

            # Reset autovacuum. The table is static, so this doesn't really
            # matter since it'll never need a vacuum.
            cur.execute(sql.SQL('''ALTER TABLE {temp_schema}.{name} RESET ( autovacuum_enabled );''')
                        .format(name=sql.Identifier(self._name), temp_schema=sql.Identifier(self._temp_schema)))
            self._conn.commit()
        ...
        try:
            ...
            with self._conn.cursor() as cur:
                cur.execute(sql.SQL('''VACUUM ANALYZE {temp_schema}.{name};''')
                            .format(name=sql.Identifier(self._name),
                                    temp_schema=sql.Identifier(self._temp_schema)))
        finally:
    ...
    def replace(self, new_last_modified):
        with self._conn.cursor() as cur:
            ...
            cur.execute(sql.SQL('''DROP TABLE IF EXISTS {schema}.{name};''')
                        .format(name=sql.Identifier(self._name),
                                schema=sql.Identifier(self._dst_schema)))
            cur.execute(sql.SQL('''ALTER TABLE {temp_schema}.{name} SET SCHEMA {schema};''')
                        .format(name=sql.Identifier(self._name),
                                temp_schema=sql.Identifier(self._temp_schema),
                                schema=sql.Identifier(self._dst_schema)))
            ...
            if cur.rowcount == 0:
                cur.execute(sql.SQL('''INSERT INTO {schema}.{metadata_table}
                                           (name, last_modified)
                                       VALUES (%s, %s)''')
                            .format(schema=sql.Identifier(self._dst_schema),
                                    metadata_table=sql.Identifier(self._metadata_table)),
                ...
                cur.execute(sql.SQL('''UPDATE {schema}.{metadata_table}
                                       SET last_modified = %s
                                       WHERE name = %s''')
                            .format(schema=sql.Identifier(self._dst_schema),
                                    metadata_table=sql.Identifier(self._metadata_table)),
    ...
    def _download(self, url, headers=None):
        if url.startswith('file://'):
            ...
            if headers and 'If-Modified-Since' in headers:
                ...
                if str(int(os.path.getmtime(filename))) == headers['If-Modified-Since']:  # Basic comparison
                    return DownloadResult(status_code=requests.codes.not_modified)
            with open(filename, 'rb') as fp:
                return DownloadResult(status_code=200, content=fp.read(),
                                      last_modified=str(os.fstat(fp.fileno()).st_mtime))  # Ensure string
        ...
        if opts.no_update and (cached_data or table_last_modified):
            ...
            if response.status_code == requests.codes.ok:
                ...
                if opts.cache:
                    ...
                    elif os.path.exists(filename_lastmod):  # if server didn't send one, remove old
                ...
                if os.path.exists(filename) and os.path.exists(filename_lastmod):
                    ...
                    logging.info("  Remote data for {} not modified based on table metadata.".format(name))
                    # We need a DownloadResult that signifies "not modified" but has the last_modified value
                    result = DownloadResult(status_code=requests.codes.not_modified, last_modified=table_last_modified)
            else:
...
def main():
    ...
    if opts.verbose:
        ...
        logging.basicConfig(format='%(levelname)s: %(messages)s', level=logging.WARNING)
    else:
        ...
        try:
            with Downloader() as d:
                ...
                for name, source in config["sources"].items():
                    ...
                    if not re.match(r'''^[a-zA-Z0-9_]+$''', name):  # r'' for raw string
                        raise RuntimeError(
                    ...
                    download_result = d.download(source["url"], name, opts, data_dir, table_lm)
                    ...
                    if download_result is None:  # Download failed critically
                        logging.warning(f"  Skipping import for table {name} due to download failure.")
                        needs_import = False
                    elif download_result.status_code == requests.codes.not_modified:
                        logging.info(
                            f"  Data for table {name} is not modified. Current LMT: {download_result.last_modified}")
                        if opts.force or opts.force_import:
                            logging.info(f"  Force importing table {name} despite no modification.")
                        else:
                            needs_import = False
                    elif not opts.force and not opts.force_import and table_lm == download_result.last_modified:
                        logging.info(
                    ...
                    if download_result.content is None and download_result.status_code == requests.codes.ok:
                        logging.error(f"  Table {name} needs import, but download content is missing unexpectedly.")
                        continue  # or raise error

                    logging.info(f"  Proceeding with import for table {name}.")
                    ...
                    if "archive" in source and source["archive"]["format"] == "zip":
                        ...
                        if download_result.content is None:
                            logging.error(f"  Cannot decompress {name}: download content is missing.")
                            continue
                        zip_file = zipfile.ZipFile(io.BytesIO(download_result.content))
                    ...
                    ogr_target_table = f"{config['settings']['temp_schema']}.{name}"

                    ogrcommand = ["ogr2ogr",
                                  '-f', 'PostgreSQL',
                                  ogrpg,  # Destination datasource name
                                  os.path.join(workingdir, source["file"]),  # Source datasource name
                                  '-lco', 'GEOMETRY_NAME=way',
                                  '-lco', 'SPATIAL_INDEX=FALSE',  # We create index later
                                  # '-lco', 'EXTRACT_SCHEMA_FROM_LAYER_NAME=YES', # This might conflict with -nln
                                  '-nln', ogr_target_table,  # Target layer name (schema.table)
                    ...
                    try:
                        ...
                        process = subprocess.run(ogrcommand, capture_output=True, text=True, check=True)
                        if process.stdout:
                            logging.debug(f"ogr2ogr stdout:\n{process.stdout}")
            ...
            logging.error(f"SQL: {e.diag.sqlstate if e.diag else 'N/A'}")  # More detailed error
            logging.error(f"Message: {e.diag.message_primary if e.diag else 'N/A'}")
