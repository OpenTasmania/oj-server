...
def test_install_script_help_output():
    ...
    if not INSTALL_SCRIPT_PATH.is_file():
        raise FileNotFoundError(f"install.py not found at {INSTALL_SCRIPT_PATH}")
    ...
    assert "Ensures 'uv' (Python packager and virtual environment manager) is installed." in result.stdout, \
    ...
    assert "Arguments for installer.main_installer (passed if --continue-install is used):" in result.stdout, \
    ...
    assert "Help information for the main setup module (installer.main_installer):" in result.stdout, \
    ...
    assert result.returncode == 0, f"install.py --help exited with code {result.returncode}"