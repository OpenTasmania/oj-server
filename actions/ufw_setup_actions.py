...
def ensure_ufw_package_installed(
    ...
    log_map_server(
        f"{config.SYMBOLS['info']} Checking UFW package installation status...",
        "info",
        logger_to_use,
    )
    if check_package_installed("ufw", current_logger=logger_to_use):
        log_map_server(
            f"{config.SYMBOLS['success']} UFW package is installed.",
            "success",
            logger_to_use,
        )
    else:
        ...
        raise EnvironmentError(
            "UFW package not found, but is a core prerequisite."
        )


def enable_ufw_service(
    current_logger: Optional[logging.Logger] = None,
) -> None:
    """
    ...
    logger_to_use = current_logger if current_logger else module_logger
    log_map_server(
        f"{config.SYMBOLS['step']} Enabling UFW service if inactive...",
        "info",
        logger_to_use,
    )
    ...
    try:
        ...
        log_map_server(
            f"{config.SYMBOLS['info']} Final UFW status:",
            "info",
            logger_to_use,
        )
        run_elevated_command(
