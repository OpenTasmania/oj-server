...
def setup_pgpass(
    ...
    pg_password = app_settings.pg.password  # This is the actual password to write
    ...
    if not can_create_pgpass:
        log_map_server(
            f"{symbols.get('info', 'ℹ️')} PGPASSWORD is not set, is default (and dev override is not active), or other issue. "
            ".pgpass file not created/updated.", "info", logger_to_use, app_settings)
        if pg_password == PGPASSWORD_DEFAULT and not allow_default_for_dev:  # Check against imported default
            log_map_server(
    ...
    try:
        ...
        if not os.path.isdir(home_dir_user_specific):
            ...
            if os.path.isdir(home_dir):
                log_map_server(
                    f"{symbols.get('info', 'ℹ️')} Using generic home directory {home_dir} for .pgpass for user {current_user_name}.",
                    "info", logger_to_use, app_settings)
            else:  # Should be rare
                log_map_server(
        ...
        if os.path.isfile(pgpass_file_path):
            ...
            except Exception as e_read:  # Catch more general errors during read
                log_map_server(
