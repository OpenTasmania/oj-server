...
import subprocess
from .. import config
from ..command_utils import run_elevated_command, log_map_server, SYMBOLS  # Removed command_exists as not used here
...
def certbot_setup(current_logger: Optional[logging.Logger] = None) -> None:
    ...
    log_map_server(f"{config.SYMBOLS['step']} Setting up SSL certificates using Certbot with Nginx plugin...", "info",
                   logger_to_use)
    ...
    if is_default_domain or is_ip_address or is_localhost or "." not in domain_to_certify:
        log_map_server(
    ...
    log_map_server(f"{config.SYMBOLS['package']} Installing Certbot and Nginx plugin...", "info", logger_to_use)
    try:
        ...
        run_elevated_command(["apt", "install", "-y", "certbot", "python3-certbot-nginx"], current_logger=logger_to_use)
    except Exception as e:
        log_map_server(f"{config.SYMBOLS['error']} Failed to install Certbot packages: {e}", "error", logger_to_use)
        raise  # Critical for this step

    admin_email = f"admin@{domain_to_certify}"  # Generate a plausible admin email
    log_map_server(
        f"{config.SYMBOLS['gear']} Running Certbot for domain: {domain_to_certify} with registration email {admin_email}...",
        "info", logger_to_use)

    certbot_cmd = [
        "certbot", "--nginx",  # Use the Nginx plugin
    ...
    try:
        ...
        run_elevated_command(certbot_cmd, capture_output=True,
                             current_logger=logger_to_use)  # capture_output helps to see certbot's own summary
        log_map_server(
        ...
        log_map_server(f"{config.SYMBOLS['error']} An unexpected error occurred during Certbot setup: {e_unexp}",
                       "error", logger_to_use)
