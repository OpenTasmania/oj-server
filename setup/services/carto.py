...
def carto_setup(current_logger: Optional[logging.Logger] = None) -> None:
    ...
    log_map_server(
        f"{config.SYMBOLS['step']} Setting up CartoCSS compiler and OpenStreetMap-Carto stylesheet...",
        "info",
        logger_to_use,
    )
    ...
    log_map_server(
        f"{config.SYMBOLS['package']} Installing CartoCSS compiler (carto) globally via npm...",
        "info",
        logger_to_use,
    )
    try:
    ...
    if dir_exists_check.returncode != 0:
        log_map_server(
            f"{config.SYMBOLS['info']} Cloning OpenStreetMap-Carto repository to {osm_carto_base_dir}...",
            "info",
            logger_to_use,
        )
        run_elevated_command(
    ...
    try:
        ...
        log_map_server(
            f"{config.SYMBOLS['gear']} Compiling project.mml to mapnik.xml (running as {current_user})...",
            "info",
            logger_to_use,
        )
        ...
        carto_cmd = [
            "carto",
            "project.mml",
        ]  # Assumes 'carto' is in PATH for the user
        ...
        with open(
            compile_log_filename, "w"
        ) as log_f:  # Write as current user (due to chown)
        ...
        if (
            mapnik_xml_created_successfully
            and os.path.isfile("mapnik.xml")
            and os.path.getsize("mapnik.xml") > 0
        ):
            ...
            log_map_server(
                f"{config.SYMBOLS['success']} mapnik.xml copied to {mapnik_style_target_dir}/",
                "success",
                logger_to_use,
            )
        ...
        log_map_server(
            f"{config.SYMBOLS['error']} Error during OpenStreetMap-Carto processing: {e_carto_processing}",
            "error",
            logger_to_use,
        )
