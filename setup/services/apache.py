...
def apache_modtile_setup(current_logger: Optional[logging.Logger] = None) -> None:
    logger_to_use = current_logger if current_logger else module_logger
    log_map_server(f"{config.SYMBOLS['step']} Setting up Apache with mod_tile...", "info", logger_to_use)

    if not command_exists("apache2ctl"):  # Check for apache2ctl as a proxy for apache2 installation
        log_map_server(f"{config.SYMBOLS['warning']} Apache2 (apache2ctl) not found. Skipping Apache/mod_tile setup.",
    ...
    if backup_file(ports_conf_path, current_logger=logger_to_use):
        ...
        run_elevated_command(["sed", "-i.bak_ports", "s/^Listen 80$/Listen 8080/", ports_conf_path],
                             current_logger=logger_to_use)
        # Also check for IPv6 Listen [::]:80
        run_elevated_command(["sed", "-i", "s/^Listen \\[::\\]:80$/Listen [::]:8080/", ports_conf_path],
                             current_logger=logger_to_use, check=False)  # Allow to fail if not present
        log_map_server(f"{config.SYMBOLS['success']} Apache configured to listen on port 8080 (was 80).", "success",
                       logger_to_use)

    mod_tile_conf_available_path = "/etc/apache2/conf-available/mod_tile.conf"
    mod_tile_conf_content = f"""# mod_tile.conf - Generated by script V{config.SCRIPT_VERSION}
...
"""
    run_elevated_command(["tee", mod_tile_conf_available_path], cmd_input=mod_tile_conf_content,
                         current_logger=logger_to_use)
    log_map_server(f"{config.SYMBOLS['success']} Created {mod_tile_conf_available_path}", "success", logger_to_use)
...
"""
    run_elevated_command(["tee", apache_tiles_site_conf_available_path], cmd_input=apache_tiles_site_content,
                         current_logger=logger_to_use)
    log_map_server(f"{config.SYMBOLS['success']} Created {apache_tiles_site_conf_available_path}", "success",
                   logger_to_use)

    apache_modules_to_enable = ["tile", "expires", "headers"]
    for mod in apache_modules_to_enable:
        log_map_server(f"{config.SYMBOLS['gear']} Enabling Apache module: {mod}...", "info", logger_to_use)
    ...
    log_map_server(
        ...
        "info", logger_to_use)
    run_elevated_command(["a2ensite", os.path.basename(apache_tiles_site_conf_available_path)],
                         current_logger=logger_to_use)

    default_site_name = "000-default.conf"
    default_site_enabled_path = f"/etc/apache2/sites-enabled/{default_site_name}"
    # Check if the default site is enabled (is a symlink)
    is_link_check = run_elevated_command(["test", "-L", default_site_enabled_path], check=False, capture_output=True,
                                         current_logger=logger_to_use)
    if is_link_check.returncode == 0:
        log_map_server(f"{config.SYMBOLS['info']} Disabling default Apache site ({default_site_name})...", "info",
                       logger_to_use)
        run_elevated_command(["a2dissite", default_site_name.replace(".conf", "")],
                             current_logger=logger_to_use)  # a2dissite usually takes name without .conf

    systemd_reload(current_logger=logger_to_use)
    log_map_server(f"{config.SYMBOLS['gear']} Restarting and enabling Apache service...", "info", logger_to_use)
    ...
    log_map_server(f"{config.SYMBOLS['success']} Apache with mod_tile setup completed.", "success", logger_to_use)