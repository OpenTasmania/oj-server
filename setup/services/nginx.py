...
from .. import config
from ..command_utils import run_elevated_command, log_map_server, command_exists
...
def nginx_setup(current_logger: Optional[logging.Logger] = None) -> None:
    ...
    log_map_server(f"{config.SYMBOLS['step']} Setting up Nginx as a reverse proxy...", "info", logger_to_use)

    if not command_exists("nginx"):
        log_map_server(f"{config.SYMBOLS['warning']} Nginx not found. Skipping Nginx setup.", "warning", logger_to_use)
        return

    test_page_base_dir = "/var/www/html"  # Base for www-data
    test_page_dir = os.path.join(test_page_base_dir, "map_test_page")  # Specific subdir
    run_elevated_command(["mkdir", "-p", test_page_dir], current_logger=logger_to_use)
...
"""
    ...
    run_elevated_command(["tee", index_html_path], cmd_input=simple_index_html_content, current_logger=logger_to_use)

    run_elevated_command(["chown", "-R", "www-data:www-data", test_page_dir], current_logger=logger_to_use)
    run_elevated_command(["chmod", "-R", "755", test_page_dir], current_logger=logger_to_use)
...
"""
    ...
    nginx_transit_proxy_conf_content = nginx_transit_proxy_conf_content.replace("$http_upgrade", "$$http_upgrade")
    nginx_transit_proxy_conf_content = nginx_transit_proxy_conf_content.replace("$host", "$$host")
    nginx_transit_proxy_conf_content = nginx_transit_proxy_conf_content.replace("$remote_addr", "$$remote_addr")
    nginx_transit_proxy_conf_content = nginx_transit_proxy_conf_content.replace("$proxy_add_x_forwarded_for",
                                                                                "$$proxy_add_x_forwarded_for")
    nginx_transit_proxy_conf_content = nginx_transit_proxy_conf_content.replace("$scheme", "$$scheme")
    nginx_transit_proxy_conf_content = nginx_transit_proxy_conf_content.replace("$uri", "$$uri")

    run_elevated_command(["tee", nginx_conf_available_path], cmd_input=nginx_transit_proxy_conf_content,
                         current_logger=logger_to_use)
    log_map_server(f"{config.SYMBOLS['success']} Created Nginx site configuration: {nginx_conf_available_path}",
    ...
    if os.path.isfile(nginx_conf_available_path):  # This check is as current user, might not see root-owned file
        # Better to rely on ln -sf to overwrite if it exists, or fail if source is missing
        run_elevated_command(["ln", "-sf", nginx_conf_available_path, nginx_conf_enabled_path],
                             current_logger=logger_to_use)
        log_map_server(f"{config.SYMBOLS['info']} Enabled Nginx site by symlinking to {nginx_conf_enabled_path}.",
                       "info", logger_to_use)
    else:
    ...
    is_link_check = run_elevated_command(["test", "-L", default_nginx_symlink], check=False, capture_output=True,
                                         current_logger=logger_to_use)
    if is_link_check.returncode == 0:  # Symlink exists
        run_elevated_command(["rm", default_nginx_symlink], current_logger=logger_to_use)
        log_map_server(f"{config.SYMBOLS['info']} Disabled default Nginx site by removing symlink.", "info",
                       logger_to_use)

    log_map_server(f"{config.SYMBOLS['gear']} Testing Nginx configuration...", "info", logger_to_use)
    try:
        run_elevated_command(["nginx", "-t"],
                             current_logger=logger_to_use)  # Will raise CalledProcessError if nginx -t fails
        ...
        log_map_server(f"{config.SYMBOLS['success']} Nginx setup completed.", "success", logger_to_use)
