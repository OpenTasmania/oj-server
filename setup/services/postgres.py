...
import logging
import os
import subprocess  # For CalledProcessError, if not fully abstracted by run_command
from typing import Optional

from .. import config  # Access config.PGUSER, config.ADMIN_GROUP_IP etc.
from ..command_utils import run_command, run_elevated_command, log_map_server
from ..helpers import backup_file, validate_cidr  # Import necessary helpers
...
def postgres_setup(current_logger: Optional[logging.Logger] = None) -> None:
    ...
    if not os.path.isdir(pg_conf_dir):
        try:
            run_elevated_command(["test", "-d", pg_conf_dir], check=True, capture_output=True,
    ...
    for cmd_sql in db_permission_commands:
        run_command(["sudo", "-u", "postgres", "psql", "-d", config.PGDATABASE, "-c", cmd_sql],
                    current_logger=logger_to_use)
