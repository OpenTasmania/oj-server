...
def pg_tileserv_setup(current_logger: Optional[logging.Logger] = None) -> None:
    logger_to_use = current_logger if current_logger else module_logger
    log_map_server(f"{config.SYMBOLS['step']} Setting up pg_tileserv...", "info", logger_to_use)
    ...
    if not command_exists(pg_tileserv_bin_path):
        ...
        try:
            ...
            run_command(["wget", config.PG_TILESERV_BINARY_LOCATION, "-O", temp_zip_path], current_logger=logger_to_use)
            ...
            run_command(["unzip", "-j", temp_zip_path, "pg_tileserv", "-d", temp_dir_extract],
            ...
            run_elevated_command(["chmod", "+x", pg_tileserv_bin_path], current_logger=logger_to_use)
            ...
            if temp_dir_extract and os.path.isdir(temp_dir_extract):
                shutil.rmtree(temp_dir_extract)  # Clean up extraction directory
    else:
...
"""
    ...
    try:
        ...
        run_elevated_command([
            ...
            "--home-dir", "/var/empty", "--user-group", pgtileserv_system_user  # Creates group with same name
        ], current_logger=logger_to_use)
...
"""
    ...
    run_elevated_command(["systemctl", "enable", "pg_tileserv.service"],
                         current_logger=logger_to_use)  # Add .service suffix
    ...
    run_elevated_command(["systemctl", "status", "pg_tileserv.service", "--no-pager", "-l"],
                         current_logger=logger_to_use)
    log_map_server(f"{config.SYMBOLS['success']} pg_tileserv setup completed.", "success", logger_to_use)