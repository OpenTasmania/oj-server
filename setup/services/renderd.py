...
def renderd_setup(current_logger: Optional[logging.Logger] = None) -> None:
    ...
    log_map_server(f"{config.SYMBOLS['step']} Setting up renderd for raster tiles...", "info", logger_to_use)
    ...
    if command_exists("mapnik-config"):  # from command_utils
        try:
            mapnik_config_res = run_command(["mapnik-config", "--input-plugins"], capture_output=True, check=True,
            ...
            log_map_server(
                f"{config.SYMBOLS['info']} Determined Mapnik plugins directory: {mapnik_plugins_dir_resolved}", "info",
                logger_to_use)
    ...
    renderd_host = config.VM_IP_OR_DOMAIN if config.VM_IP_OR_DOMAIN != config.VM_IP_OR_DOMAIN_DEFAULT else "localhost"
...
"""
    try:
        ...
        log_map_server(f"{config.SYMBOLS['success']} Created/Updated {renderd_conf_path}", "success", logger_to_use)
    except Exception as e:
        log_map_server(f"{config.SYMBOLS['error']} Failed to write {renderd_conf_path}: {e}", "error", logger_to_use)
        raise
...
"""
    ...
    log_map_server(f"{config.SYMBOLS['gear']} Creating necessary directories and setting permissions for renderd...",
                   "info", logger_to_use)
    run_elevated_command(["mkdir", "-p", "/var/lib/mod_tile"], current_logger=logger_to_use)
    # /var/run/renderd should be handled by RuntimeDirectory in systemd service, but mkdir -p is harmless.
    run_elevated_command(["mkdir", "-p", "/var/run/renderd"], current_logger=logger_to_use)
    run_elevated_command(["chown", "-R", "www-data:www-data", "/var/lib/mod_tile"], current_logger=logger_to_use)
    run_elevated_command(["chown", "-R", "www-data:www-data", "/var/run/renderd"], current_logger=logger_to_use)

    systemd_reload(current_logger=logger_to_use)
    log_map_server(f"{config.SYMBOLS['gear']} Enabling and restarting renderd service...", "info", logger_to_use)
    run_elevated_command(["systemctl", "enable", "renderd.service"], current_logger=logger_to_use)  # Add .service
    ...
    log_map_server(f"{config.SYMBOLS['success']} Renderd setup complete.", "success", logger_to_use)