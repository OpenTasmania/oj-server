...
def cli_prompt_for_rerun(
    ...
    try:
        user_input = input(f"   {symbols.get('info', 'ℹ️')} {prompt_message} (y/N): ").strip().lower()
        return user_input == "y"
...
def view_configuration(
    ...
    if app_config.pg.password == PGPASSWORD_DEFAULT and not app_config.dev_override_unsafe_password:  # PGPASSWORD_DEFAULT from config_models
        pg_password_display = "[DEFAULT - Potentially Insecure! Override via ENV, YAML, or CLI -W]"
    elif app_config.pg.password == PGPASSWORD_DEFAULT and app_config.dev_override_unsafe_password:
    ...
    log_map_server("Displaying current configuration:", "info", logger_to_use, app_config)
    print(f"\n{config_text}\n")
