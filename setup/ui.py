...
from .state_manager import is_step_completed, mark_step_completed, view_completed_steps, clear_state_file
...
def execute_step(step_tag: str, step_description: str, step_function: Callable, current_logger_instance) -> bool:
    """
    ...
    if is_step_completed(step_tag, current_logger=logger_to_use):
        ...
        log_map_server(
            f"{config.SYMBOLS['info']} Step '{step_description}' ({step_tag}) is already marked as completed.", "info",
            logger_to_use)
        try:
            user_input = input(f"   {config.SYMBOLS['info']} Do you want to re-run it anyway? (y/N): ").strip().lower()
        except EOFError:
            user_input = 'n'
            log_map_server(f"{config.SYMBOLS['warning']} No user input (EOF), defaulting to skip re-run.", "warning",
                           logger_to_use)

        if user_input != 'y':
            log_map_server(f"{config.SYMBOLS['info']} Skipping step: {step_tag} - {step_description}", "info",
                           logger_to_use)
            run_this_step = False

    if run_this_step:
        log_map_server(f"--- {config.SYMBOLS['step']} Executing: {step_description} ({step_tag}) ---", "info",
                       logger_to_use)
        try:
            ...
            log_map_server(f"{config.SYMBOLS['error']} FAILED: {step_description} ({step_tag})", "error", logger_to_use)
...
def view_configuration(current_logger=None) -> None:
    ...
    elif not config.PGPASSWORD:  # Handles if PGPASSWORD was set to empty string
...
def manage_state_interactive(current_logger) -> None:
    ...
    while True:
        ...
        if choice == "1":
            ...
            if completed:
                print("\nCompleted Steps:")
                for i, step_tag in enumerate(completed): print(f"  {i + 1}. {step_tag}")
            else:
                print("\nNo steps have been marked as completed yet.")
        elif choice == "2":
            confirm = input(
                f"{config.SYMBOLS['warning']} Are you sure you want to clear recorded progress from {config.STATE_FILE_PATH}? (yes/NO): ").strip().lower()
            if confirm == 'yes':
                clear_state_file(write_version_only=True, current_logger=logger_to_use)  # Keep version
...
def run_custom_selection_interactive(all_steps: List[Tuple[str, str, Callable]], current_logger) -> None:
    ...
    log_map_server("Custom step selection needs its SYMBOLS usage reviewed if any direct use.", "warning",
                   current_logger)
    print("Custom step selection placeholder - implement fully or remove if not using interactive menu.")


def show_menu(all_step_definitions: List[Tuple[str, str, Callable]], current_logger) -> None:
    """Display the main menu and handle user input."""
    logger_to_use = current_logger if current_logger else module_logger

    while True:
        print("\n" + "=" * 80)
        print(f"Map Server Setup Script (v{config.SCRIPT_VERSION}) - Main Menu")
        print("=" * 80)
        ...
        #     run_custom_selection_interactive(all_step_definitions, current_logger=logger_to_use)
            ...
            if confirm_exit == 'y':
                log_map_server("Exiting script by user choice from menu.", "info", logger_to_use)
                break
        else:
