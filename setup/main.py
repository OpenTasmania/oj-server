...
from . import config
from .command_utils import log_map_server, command_exists, check_package_installed
...
from .services.service_orchestrator import services_setup_group
from .data_processing import data_prep_group
...
def systemd_reload_step_group(current_logger) -> bool:
    return execute_step("SYSTEMD_RELOAD_MAIN", "Reload Systemd Daemon",
                        lambda cl_lambda: systemd_reload(current_logger=cl_lambda),  # Lambda correctly takes a logger
                        current_logger)


def main_map_server_entry() -> int:
    parser = argparse.ArgumentParser(
        description="Map Server Setup Script. Automates installation and configuration.",
        epilog="Example: ./install_map_server.py --full -v mymap.example.com",
    ...
    parser.add_argument("--systemd-reload", dest="systemd_reload_flag", action="store_true",
    ...
    parser.add_argument("--im-a-developer-get-me-out-of-here", "--dev-override-unsafe-password",
                        action="store_true",
    ...
    try:
        ...
        print(f"DEBUG SYS.ARGV for setup/main.py (before parse_args): {sys.argv}", file=sys.stderr)
        ...
        print(f"DEBUG Parsed args object: {args}", file=sys.stderr)  # For checking parsed values
        print(f"DEBUG args.dev_override_unsafe_password value: {args.dev_override_unsafe_password}", file=sys.stderr)
        ...
        print(f"\n--- ArgParse Error Exit Intercepted in setup/main.py ---", file=sys.stderr)
        ...
        print(f"--- End ArgParse Error Context ---", file=sys.stderr)
        return e.code
    ...
    log_map_server(f"DEBUG config.DEV_OVERRIDE_UNSAFE_PASSWORD is now: {config.DEV_OVERRIDE_UNSAFE_PASSWORD}", "info",
                   current_logger=logger)

    if config.PGPASSWORD == config.PGPASSWORD_DEFAULT and not args.view_config and not config.DEV_OVERRIDE_UNSAFE_PASSWORD:
        ...
        log_map_server(
            f"   Provide a password via -W option or use --dev-override-unsafe-password (at your own risk) to use the default for .pgpass.",
            "warning", current_logger=logger)
    ...
    if os.geteuid() != 0:
        ...
        log_map_server(f"{config.SYMBOLS['info']} Script is running as root. Privileged operations will run directly.",
                       "info", current_logger=logger)
    ...
    if args.view_state:
        ...
        if completed:
            log_map_server(f"{config.SYMBOLS['info']} Completed steps recorded in state file:", "info",
                           current_logger=logger)
            for s_idx, s_item in enumerate(completed): print(f"  {s_idx + 1}. {s_item}")
        else:
    ...
    if args.full:
        ...
        if overall_success: overall_success = overall_success and prereqs_install_group(current_logger=logger)
        if overall_success: overall_success = overall_success and services_setup_group(current_logger=logger)
        if overall_success: overall_success = overall_success and systemd_reload_step_group(current_logger=logger)
        if overall_success: overall_success = overall_success and data_prep_group(current_logger=logger)
        ...
        action_taken = True
        log_map_server(f"{config.SYMBOLS['rocket']}====== Running Systemd Reload Only ======", current_logger=logger)
    ...
    if not action_taken:
        ...
        if args is not None:
            ...
            print("Error: Argument parsing failed before args object was created.", file=sys.stderr)
    ...
    if not overall_success:
        ...
        log_map_server(f"{config.SYMBOLS['sparkles']} All requested operations completed.", "success",
                       current_logger=logger)
        if action_taken and not args.full:
            log_map_server(f"{config.SYMBOLS['info']} Partial run completed successfully.", "info",
                           current_logger=logger)
    return 0