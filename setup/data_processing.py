...
from .ui import (
    execute_step,
)  # To call individual steps if this module had sub-steps
...
try:
    ...
    logging.getLogger(
        __name__
    ).warning(  # Use a basic logger if this module is imported early
...
def gtfs_data_prep(current_logger=None) -> None:
    ...
    try:
        ...
        log_map_server(
            f"{config.SYMBOLS['info']} Ensured GTFS log file exists and is writable: {gtfs_log_file}",
            "info",
            logger_to_use,
        )
    ...
    if not gtfs_main_pipeline or not gtfs_utils:
        ...
        log_map_server(
            "   This script needs `gtfs_processor` to be in its Python environment.",
            "error",
            logger_to_use,
        )
        log_map_server(
            "   NEXT STEP: Refactor this to use `uv` to run `gtfs_processor` in an isolated environment.",
            "error",
            logger_to_use,
        )
        raise ImportError(
    ...
    try:
        log_map_server(
            f"{config.SYMBOLS['info']} Setting up logging for gtfs_processor module...",
            "info",
            logger_to_use,
        )
        ...
        if success:
            ...
            log_map_server(
                f"{config.SYMBOLS['info']} Verifying data import (counts from tables)...",
                "info",
                logger_to_use,
            )
            try:
            ...
            log_map_server(
                f"{config.SYMBOLS['error']} GTFS ETL pipeline FAILED. Check {gtfs_log_file}.",
                "error",
                logger_to_use,
            )
        ...
        log_map_server(
            f"{config.SYMBOLS['error']} Critical error importing/using `gtfs_processor`: {e}",
            "error",
            logger_to_use,
        )
        raise
        ...
        log_map_server(
            f"{config.SYMBOLS['error']} An unexpected error occurred during GTFS processing: {e}",
            "error",
            logger_to_use,
        )
        raise
    ...
    log_map_server(
        f"{config.SYMBOLS['gear']} Setting up cron job for daily GTFS updates...",
        "info",
        logger_to_use,
    )
    ...
    if not python_executable:
        log_map_server(
            f"{config.SYMBOLS['error']} Python executable not found. Cannot set up cron job.",
            "error",
            logger_to_use,
        )
        return
    ...
    try:
        ...
        run_elevated_command(
            ["crontab", "-u", cron_user_for_gtfs, temp_cron_path],
            current_logger=logger_to_use,
        )
        log_map_server(
        ...
        log_map_server(
            f"{config.SYMBOLS['error']} Failed to set up cron job for GTFS update: {e_cron}",
            "error",
            logger_to_use,
        )
    finally:
...
def data_prep_group(current_logger) -> bool:
    ...
    for tag, desc, func in step_definitions_in_group:
        if not execute_step(
            ...
            log_map_server(
                f"{config.SYMBOLS['error']} Step '{desc}' failed. Aborting data prep group.",
                "error",
                logger_to_use,
            )
            break
