...
def initialize_state_system(current_logger=None) -> None:
    ...
    if not os.path.isdir(state_dir):
        log_map_server(f"{SYMBOLS['info']} Creating state directory: {state_dir}", "info", logger_to_use)
        run_elevated_command(["mkdir", "-p", state_dir], current_logger=logger_to_use)
        run_elevated_command(["chmod", "750", state_dir], current_logger=logger_to_use)  # Group access

    if not os.path.isfile(STATE_FILE_PATH):
        log_map_server(f"{SYMBOLS['info']} Initializing state file: {STATE_FILE_PATH}", "info", logger_to_use)
        ...
        try:
            run_elevated_command(["cp", temp_file_path, STATE_FILE_PATH], current_logger=logger_to_use)
            run_elevated_command(["chmod", "640", STATE_FILE_PATH], current_logger=logger_to_use)  # Group read
        finally:
            os.unlink(temp_file_path)
    else:
        try:
            # Grep needs capture_output=True to read its stdout
            result = run_elevated_command(["grep", "^# Script Version:", STATE_FILE_PATH],
            ...
            if result.returncode == 0 and result.stdout:
                ...
                if stored_version_match:
                    ...
                    if stored_version != SCRIPT_VERSION:
                        ...
                        log_map_server(f"{SYMBOLS['info']} Clearing state file due to version mismatch.", "info",
                                       logger_to_use)
                        clear_state_file(write_version_only=True, current_logger=logger_to_use)
                else:
                    log_map_server(
                        f"{SYMBOLS['warning']} State file version line not found or malformed. Re-initializing.",
                        "warning", logger_to_use)
                    clear_state_file(write_version_only=True, current_logger=logger_to_use)
            elif result.returncode == 1:  # Grep found nothing (empty file or no version line)
                log_map_server(
...
def mark_step_completed(step_tag: str, current_logger=None) -> None:
    ...
    try:
        ...
        result = run_elevated_command(["grep", "-Fxq", step_tag, STATE_FILE_PATH],
                                      check=False, capture_output=True, current_logger=logger_to_use)
        if result.returncode != 0:  # Step tag not found, so add it
            log_map_server(f"{SYMBOLS['info']} Marking step '{step_tag}' as completed.", "info", logger_to_use)
            run_elevated_command(["tee", "-a", STATE_FILE_PATH], cmd_input=f"{step_tag}\n",
                                 capture_output=False, current_logger=logger_to_use)
        else:
            log_map_server(f"{SYMBOLS['info']} Step '{step_tag}' was already marked as completed.", "info",
                           logger_to_use)
    except Exception as e:
        log_map_server(f"{SYMBOLS['error']} Error marking step '{step_tag}': {e}", "error", logger_to_use)
...
def clear_state_file(write_version_only: bool = False, current_logger=None) -> None:
    """Clear the state file, optionally keeping only the version line."""
    logger_to_use = current_logger if current_logger else module_logger
    log_map_server(f"{SYMBOLS['info']} Clearing state file: {STATE_FILE_PATH}", "info", logger_to_use)
    ...
    try:
        ...
        run_elevated_command(["cp", temp_file_path, STATE_FILE_PATH], current_logger=logger_to_use)
        if not write_version_only:
            log_map_server(f"{SYMBOLS['success']} Progress state file cleared.", "success", logger_to_use)
        else:
            log_map_server(f"{SYMBOLS['success']} Progress state file re-initialized with version.", "success",
                           logger_to_use)
    except Exception as e:
        log_map_server(f"{SYMBOLS['error']} Failed to clear state file: {e}", "error", logger_to_use)
    finally:
...
def view_completed_steps(current_logger=None) -> List[str]:
    ...
    try:
        ...
        result = run_elevated_command(["grep", "-v", "^#", STATE_FILE_PATH],
                                      capture_output=True, check=False, current_logger=logger_to_use)
        ...
        if result.returncode == 0 and result.stdout and result.stdout.strip():
            ...
            log_map_server(
                ...
                "warning", logger_to_use)
            if result.stderr:
                log_map_server(f"   grep stderr: {result.stderr.strip()}", "warning", logger_to_use)
            return []
    except subprocess.CalledProcessError as e:  # Should be caught by run_elevated_command, but defensive
        log_map_server(f"{SYMBOLS['error']} CalledProcessError viewing completed steps: {e}", "error", logger_to_use)
        return []
    except Exception as e:
        log_map_server(f"{SYMBOLS['error']} Unexpected error viewing completed steps: {e}", "error", logger_to_use)
        return []
