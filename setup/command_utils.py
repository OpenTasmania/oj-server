...
def log_map_server(
    ...
    while _logger_to_check_handlers:
        if _logger_to_check_handlers.handlers:
            ...
            break
        if (
            not _logger_to_check_handlers.propagate
    ...
    if not has_handlers:
        ...
        if (
            effective_logger is module_logger
        ):  # Only mark if it's this module's logger
...
def run_command(
    ...
    if shell:
        ...
        command_to_log_str = str(
            command_to_run
        )  # Ensure it's a string for logging
    else:  # not shell
...
def check_package_installed(
    ...
    try:
        ...
        FileNotFoundError
    ):  # Should be caught by run_command if dpkg-query is missing
        log_map_server(
            f"{config.SYMBOLS['error']} dpkg-query command not found. Cannot check package '{package}'.",
            "error",
            logger_to_use,
        )
        return False
        ...
        log_map_server(
            f"{config.SYMBOLS['error']} Error checking if package '{package}' is installed: {e}",
            "error",
            logger_to_use,
        )
        return False
