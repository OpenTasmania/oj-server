...
def apply_ufw_rules(
    ...
    if not validate_cidr(
        ...
        log_map_server(
            f"{symbols.get('error', '❌')} {msg}", "error", logger_to_use, app_settings # Corrected: Added emoji, Pass app_settings
        )
    ...
    try:
        ...
        run_elevated_command(
            ["ufw", "allow", "in", "on", "lo"], app_settings, current_logger=logger_to_use # Pass app_settings
        )
        run_elevated_command(
            ["ufw", "allow", "out", "on", "lo"], app_settings, current_logger=logger_to_use # Pass app_settings
        )

        admin_rules = [
            (
                [
                    "ufw", "allow", "from", admin_group_ip, "to", "any", "port", "22", "proto", "tcp",
        ...
        for cmd_list, desc in admin_rules:
            ...
            run_elevated_command(cmd_list, app_settings, current_logger=logger_to_use) # Pass app_settings
        ...
        log_map_server(
            f"{symbols.get('error', '❌')} A UFW command failed during rule application. Command: '{e.cmd}', Error: {e.stderr or e.stdout}", # Corrected: Added emoji
            "error",
            logger_to_use,
            app_settings, # Pass app_settings
        )
        ...
        log_map_server(
            f"{symbols.get('error', '❌')} An unexpected error occurred during UFW rule application: {e}", # Corrected: Added emoji
            "error",
            logger_to_use,
            app_settings, # Pass app_settings
        )
...
def activate_ufw_service(
    ...
    try:
        ...
        if status_result.stdout and "inactive" in status_result.stdout.lower():
            ...
            systemd_reload(app_settings=app_settings, current_logger=logger_to_use) # Already correct
            ...
            log_map_server(
                f"{symbols.get('info', 'ℹ️')} UFW status not 'inactive'. Current status command output: STDOUT='{status_output}', STDERR='{stderr_output}', RC={status_result.returncode}. Assuming active or managed.", # Corrected: Added emoji
                "info",
                logger_to_use,
                app_settings, # Pass app_settings
            )
        ...
        run_elevated_command(
            ["ufw", "status", "verbose"], app_settings, current_logger=logger_to_use # Pass app_settings
        )

    except subprocess.CalledProcessError as e:
        log_map_server(
            f"{symbols.get('error', '❌')} A UFW command failed during service activation. Command: '{e.cmd}', Error: {e.stderr or e.stdout}", # Corrected: Added emoji
            "error",
            logger_to_use,
            app_settings, # Pass app_settings
        )
        ...
        log_map_server(
            f"{symbols.get('error', '❌')} An unexpected error occurred during UFW service activation: {e}", # Corrected: Added emoji
            "error",
            logger_to_use,
            app_settings, # Pass app_settings
        )
