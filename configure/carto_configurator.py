...
import shutil # Ensure shutil is imported for backup logic
import datetime # Ensure datetime is imported for backup logic
import re # Ensure re is imported for MML processing logic

# ... (module_logger, constants) ...
...
def compile_osm_carto_stylesheet(
    ...
    compiled_mapnik_xml_path = carto_base_path / "mapnik.xml" # Define this path early
    ...
    try:
        ...
        if not carto_base_path.is_dir():
            log_map_server(f"{symbols.get('error', '❌')} Carto base directory {carto_base_path} not found.", "error", logger_to_use, app_settings)
            raise FileNotFoundError(f"Carto base directory {carto_base_path} not found.")

        os.chdir(carto_base_path)

        if not project_mml_path.is_file():
            log_map_server(f"{symbols.get('error', '❌')} {project_mml_path} not found. Cannot compile.", "error", logger_to_use, app_settings)
        ...
        try:
            ...
            log_map_server(f"{symbols.get('warning', '!')} Could not backup {project_mml_path}: {e_backup}", "warning", logger_to_use, app_settings)

        mml_content_original_text = project_mml_path.read_text(encoding="utf-8") # Assigned here
            ...
            if not processed_datasource_block:
                ...
                if match_ds_start:
                    ...
                    while block_line_idx_iter < len(original_lines):
                        ...
                        current_line_actual_indent = current_line_actual_indent_match.group(1) if current_line_actual_indent_match else ""
                        if current_block_line_content.strip() and len(current_line_actual_indent) <= len(block_initial_indent):
                            break
                        ...
                        if kv_match_obj:
                            ...
                            if key in db_params_from_config:
                                if is_complex: logger_to_use.warning(f"MML line for DB param '{key}' ('{current_block_line_content}') has trailing data. Will be replaced cleanly.")
                            elif not is_complex: current_block_original_params[key] = val if val is not None else ""
                            else: other_structural_lines_in_block.append(current_block_line_content)
                        elif current_block_line_content.strip().startswith("#") or not current_block_line_content.strip():
                            other_structural_lines_in_block.append(current_block_line_content)
                        else: other_structural_lines_in_block.append(current_block_line_content)
                        block_line_idx_iter += 1
                    params_to_write: List[Tuple[str, str]] = [("type", current_block_original_params.pop("type", "postgis"))]
                    for db_key_ordered in ["host", "port", "dbname", "user", "password"]:
                        ...
                        current_block_original_params.pop(db_key_ordered, None)
                    for other_k, other_v in sorted(current_block_original_params.items()): params_to_write.append((other_k, other_v))
                    for k_out, v_out in params_to_write: output_lines_collector.append(f'{param_reconstruction_indent}{k_out}: "{v_out}"')
                    output_lines_collector.extend(other_structural_lines_in_block)
                    line_idx = block_line_idx_iter - 1
                else: output_lines_collector.append(line_content)
            else: output_lines_collector.append(line_content)
            line_idx += 1
        if not processed_datasource_block:
        ...
        if mml_content_original_text and mml_content_original_text.endswith("\n") and \
           not mml_content_updated_for_log.endswith("\n") and \
           mml_content_updated_for_log:
            mml_content_updated_for_log += "\n"

        project_mml_path.write_text(mml_content_updated_for_log, encoding="utf-8")
        log_map_server(f"DB parameters processing in {project_mml_path} complete.", "debug", logger_to_use, app_settings)

        carto_cmd = ["carto", "project.mml"]
        carto_result = run_command(
            carto_cmd, app_settings, capture_output=True, check=False, current_logger=logger_to_use
        )
        ...
        if carto_result.returncode == 0 and carto_result.stdout:
            ...
            logger_to_use.debug(f"MML content written to {project_mml_path} that may have caused error:\n{mml_content_updated_for_log}")
        ...
        if mml_content_updated_for_log:
            ...
            logger_to_use.debug("Neither updated nor original MML content was available at time of error (it was None or empty).")
        raise
    finally:
