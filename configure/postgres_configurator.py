...
def _get_pg_config_path_params(app_settings: AppSettings) -> tuple[str, str, str, str]:
...
def _check_pg_config_dir_exists(app_settings: AppSettings, current_logger: Optional[logging.Logger] = None) -> None:
    ...
    if not os.path.isdir(pg_conf_dir):
        try:
            ...
            log_map_server(
                ...
                f"Is PostgreSQL v{pg_version} installed?", "error", logger_to_use,
            )
...
def create_postgres_user_and_db(app_settings: AppSettings, current_logger: Optional[logging.Logger] = None) -> None:
    ...
    try:
        log_map_server(f"{symbols.get('gear', '')} Creating PostgreSQL user '{pg_user}'...", "info", logger_to_use)
        run_command(
            ["sudo", "-u", "postgres", "psql", "-c", f"CREATE USER {pg_user} WITH PASSWORD '{pg_password}';"],
            capture_output=True, current_logger=logger_to_use
        )
        log_map_server(f"{symbols.get('success', '')} PostgreSQL user '{pg_user}' created.", "success", logger_to_use)
    except subprocess.CalledProcessError as e:
        if e.stderr and "already exists" in e.stderr.lower():
            log_map_server(
                ...
                "info", logger_to_use)
            run_command(
                ["sudo", "-u", "postgres", "psql", "-c", f"ALTER USER {pg_user} WITH PASSWORD '{pg_password}';"],
                capture_output=True, current_logger=logger_to_use
            )
            log_map_server(f"{symbols.get('success', '')} Password for PostgreSQL user '{pg_user}' updated.", "success",
                           logger_to_use)
        else:
    ...
    try:
        log_map_server(f"{symbols.get('gear', '')} Creating PostgreSQL database '{pg_database}'...", "info",
                       logger_to_use)
        run_command(
        ...
        log_map_server(f"{symbols.get('success', '')} PostgreSQL database '{pg_database}' created.", "success",
                       logger_to_use)
    except subprocess.CalledProcessError as e:
        if e.stderr and "already exists" in e.stderr.lower():
            log_map_server(f"{symbols.get('info', '')} PostgreSQL database '{pg_database}' already exists.", "info",
                           logger_to_use)
        else:
...
def enable_postgres_extensions(app_settings: AppSettings, current_logger: Optional[logging.Logger] = None) -> None:
    ...
    for ext in extensions:
        ...
        try:
            run_command(
                ["sudo", "-u", "postgres", "psql", "-d", pg_database, "-c", f"CREATE EXTENSION IF NOT EXISTS {ext};"],
                current_logger=logger_to_use
            )
            log_map_server(f"{symbols.get('success', '')} PostgreSQL extension '{ext}' ensured.", "success",
                           logger_to_use)
        except subprocess.CalledProcessError as e:
            err_msg = e.stderr.strip() if e.stderr else f"Unknown psql error enabling extension {ext}."
            log_map_server(
                f"{symbols.get('error', '')} Failed to enable PostgreSQL extension '{ext}'. Error: {err_msg}", "error",
                logger_to_use)
            raise


def set_postgres_permissions(app_settings: AppSettings, current_logger: Optional[logging.Logger] = None) -> None:
    ...
    try:
        for cmd_sql in db_permission_commands:
            run_command(
                ["sudo", "-u", "postgres", "psql", "-d", pg_database, "-c", cmd_sql],
                current_logger=logger_to_use,
            )
        log_map_server(f"{symbols.get('success', '')} Database permissions set for user '{pg_user}'.", "success",
                       logger_to_use)
    except subprocess.CalledProcessError as e:
        err_msg = e.stderr.strip() if e.stderr else "Unknown psql error setting permissions."
        log_map_server(f"{symbols.get('error', '')} Failed to set PostgreSQL permissions. Error: {err_msg}", "error",
                       logger_to_use)
        raise


def customize_postgresql_conf(app_settings: AppSettings, current_logger: Optional[logging.Logger] = None) -> None:
    logger_to_use = current_logger if current_logger else module_logger
    _check_pg_config_dir_exists(app_settings, logger_to_use)

    _, _, pg_conf_file, _ = _get_pg_config_path_params(app_settings)
    symbols = app_settings.symbols
    script_hash = get_current_script_hash(project_root_dir=static_config.OSM_PROJECT_ROOT,
    ...
    if backup_file(pg_conf_file, current_logger=logger_to_use):
        ...
        customisation_marker = "# --- TRANSIT SERVER CUSTOMISATIONS - Appended by script V"
        ...
        if grep_result.returncode != 0:  # Marker not found, append the settings
            try:
                ...
                content_to_append_final = conf_additions_template.format(script_hash=script_hash)

                run_elevated_command(["tee", "-a", pg_conf_file], cmd_input=content_to_append_final,
                                     current_logger=logger_to_use)
                log_map_server(
                ...
                log_map_server(f"{symbols.get('error', '')} Error updating {pg_conf_file}: {e}", "error", logger_to_use)
                raise
        else:
            log_map_server(
...
def customize_pg_hba_conf(app_settings: AppSettings, current_logger: Optional[logging.Logger] = None) -> None:
    logger_to_use = current_logger if current_logger else module_logger
    _check_pg_config_dir_exists(app_settings, logger_to_use)
    ...
    if not validate_cidr(app_settings.admin_group_ip, current_logger=logger_to_use):
        log_map_server(
            f"{symbols.get('error', '')} Invalid ADMIN_GROUP_IP '{app_settings.admin_group_ip}' for pg_hba.conf. Skipping HBA update.",
            "error", logger_to_use)
        raise ValueError(f"Invalid ADMIN_GROUP_IP '{app_settings.admin_group_ip}' for pg_hba.conf.")

    if backup_file(pg_hba_file, current_logger=logger_to_use):
        try:
            hba_content_final = hba_template.format(**format_vars)
            run_elevated_command(["tee", pg_hba_file], cmd_input=hba_content_final, current_logger=logger_to_use)
            log_map_server(f"{symbols.get('success', '')} Wrote pg_hba.conf using template from configuration.",
                           "success", logger_to_use)
        except KeyError as e_key:
            log_map_server(
            ...
            log_map_server(f"{symbols.get('error', '')} Error writing {pg_hba_file}: {e}", "error", logger_to_use)
            raise


def restart_and_enable_postgres_service(app_settings: AppSettings,
                                        current_logger: Optional[logging.Logger] = None) -> None:
    logger_to_use = current_logger if current_logger else module_logger
    symbols = app_settings.symbols
    log_map_server(f"{symbols.get('gear', '')} Restarting and enabling PostgreSQL service...", "info", logger_to_use)
    try:
        ...
        log_map_server(f"{symbols.get('info', '')} PostgreSQL service status:", "info", logger_to_use)
        run_elevated_command(["systemctl", "status", "postgresql", "--no-pager", "-l"], current_logger=logger_to_use)
        log_map_server(f"{symbols.get('success', '')} PostgreSQL service restarted and enabled.", "success",
                       logger_to_use)
    except subprocess.CalledProcessError as e:
        log_map_server(
