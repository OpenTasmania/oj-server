...
def create_pg_tileserv_config_file(
    ...
    run_elevated_command(["mkdir", "-p", str(config_dir)], app_settings, current_logger=logger_to_use)
    ...
    if app_settings.pg.password == PGPASSWORD_DEFAULT and not app_settings.dev_override_unsafe_password:
        log_map_server(
    ...
    format_vars = {
        ...
        "pg_tileserv_development_mode_bool": str(pgts_settings.development_mode).lower(),  # bool to "true"/"false"
        "pg_tileserv_allow_function_sources_bool": str(pgts_settings.allow_function_sources).lower(),
        # bool to "true"/"false"
    }

    try:
        pg_tileserv_config_content_final = config_template_str.format(**format_vars)
        run_elevated_command(
            ["tee", str(config_file_path)], app_settings,
            cmd_input=pg_tileserv_config_content_final, current_logger=logger_to_use
        )
        log_map_server(f"{symbols.get('success', '✅')} Created/Updated {config_file_path}", "success", logger_to_use,
                       app_settings)

        # Set ownership and permissions for config file
        system_user = pgts_settings.system_user
        run_elevated_command(["chown", f"{system_user}:{system_user}", str(config_file_path)], app_settings,
                             current_logger=logger_to_use)
        run_elevated_command(["chmod", "640", str(config_file_path)], app_settings,
                             current_logger=logger_to_use)  # Readable by owner and group
        log_map_server(f"{symbols.get('success', '✅')} Permissions set for {config_file_path}.", "success",
        ...
        log_map_server(f"{symbols.get('error', '❌')} Failed to write pg_tileserv config: {e}", "error", logger_to_use,
...
def activate_pg_tileserv_service(
    ...
    logger_to_use = current_logger if current_logger else module_logger
    symbols = app_settings.symbols
    log_map_server(f"{symbols.get('step', '➡️')} Activating pg_tileserv systemd service...", "info", logger_to_use,
                   app_settings)

    systemd_reload(app_settings, current_logger=logger_to_use)
    run_elevated_command(["systemctl", "enable", "pg_tileserv.service"], app_settings, current_logger=logger_to_use)
    run_elevated_command(["systemctl", "restart", "pg_tileserv.service"], app_settings, current_logger=logger_to_use)

    log_map_server(f"{symbols.get('info', 'ℹ️')} pg_tileserv service status:", "info", logger_to_use, app_settings)
    run_elevated_command(["systemctl", "status", "pg_tileserv.service", "--no-pager", "-l"], app_settings,
                         current_logger=logger_to_use)
    log_map_server(f"{symbols.get('success', '✅')} pg_tileserv service activated.", "success", logger_to_use,
                   app_settings)