...
RENDERD_SYSTEM_GROUP = "www-data"  # Group for config file readability, matches installer


def get_mapnik_plugin_dir(app_settings: AppSettings, current_logger: Optional[logging.Logger] = None) -> str:
    ...
    if app_settings.renderd.mapnik_plugins_dir_override:
        ...
        log_map_server(f"{symbols.get('info', 'ℹ️')} Using Mapnik plugins directory from override: {override_path}",
    ...
    default_debian_plugins_dir = "/usr/lib/mapnik/input/"  # Generic for Mapnik 3.x/4.x
    ...
    if command_exists("mapnik-config"):
        try:
            mapnik_config_res = run_command(
                ["mapnik-config", "--input-plugins"], app_settings,  # Pass app_settings
    ...
    if Path(default_debian_plugins_dir).is_dir():
        log_map_server(
            f"{symbols.get('info', 'ℹ️')} Using default Mapnik plugins directory: {default_debian_plugins_dir}", "info",
...
def create_renderd_conf_file(app_settings: AppSettings, current_logger: Optional[logging.Logger] = None) -> None:
    ...
    log_map_server(f"{symbols.get('step', '➡️')} Creating {RENDERD_CONF_FILE_SYSTEM_PATH} from template...", "info",
                   logger_to_use, app_settings)
    ...
    if app_settings.renderd.num_threads_multiplier > 0:
        cpu_c = os.cpu_count()
        if cpu_c:
            num_threads_val = int(cpu_c * app_settings.renderd.num_threads_multiplier)
        else:  # Fallback if cpu_count() is None
            num_threads_val = int(2 * app_settings.renderd.num_threads_multiplier)  # e.g. 2*2=4
        if num_threads_val == 0: num_threads_val = 2  # Ensure at least 2 if multiplier is very small leading to 0
    ...
    num_threads_renderd_str = str(num_threads_val) if app_settings.renderd.num_threads_multiplier > 0 else "0"

    mapnik_plugins_dir_val = get_mapnik_plugin_dir(app_settings, logger_to_use)

    renderd_host_val = app_settings.vm_ip_or_domain
    if renderd_host_val == VM_IP_OR_DOMAIN_DEFAULT:  # Compare with imported default
    ...
    format_vars = {
        ...
        "mapnik_xml_stylesheet_path": str(app_settings.renderd.mapnik_xml_stylesheet_path),
        "renderd_host": renderd_host_val,
    }

    try:
        renderd_conf_content_final = renderd_conf_template_str.format(**format_vars)
        run_elevated_command(
            ["tee", RENDERD_CONF_FILE_SYSTEM_PATH], app_settings,
            cmd_input=renderd_conf_content_final, current_logger=logger_to_use
        )
        # Config file should be readable by the renderd user (e.g., www-data)
        run_elevated_command(["chown", f"root:{RENDERD_SYSTEM_GROUP}", RENDERD_CONF_FILE_SYSTEM_PATH], app_settings,
                             current_logger=logger_to_use)
        run_elevated_command(["chmod", "640", RENDERD_CONF_FILE_SYSTEM_PATH], app_settings,
                             current_logger=logger_to_use)
        log_map_server(f"{symbols.get('success', '✅')} Created/Updated and secured {RENDERD_CONF_FILE_SYSTEM_PATH}",
...
def activate_renderd_service(app_settings: AppSettings, current_logger: Optional[logging.Logger] = None) -> None:
    """Reloads systemd, enables and restarts the renderd service."""
    logger_to_use = current_logger if current_logger else module_logger
    symbols = app_settings.symbols
    log_map_server(f"{symbols.get('step', '➡️')} Activating Renderd systemd service...", "info", logger_to_use,
                   app_settings)

    systemd_reload(app_settings, current_logger=logger_to_use)  # Pass app_settings
    run_elevated_command(["systemctl", "enable", "renderd.service"], app_settings, current_logger=logger_to_use)
    run_elevated_command(["systemctl", "restart", "renderd.service"], app_settings, current_logger=logger_to_use)

    log_map_server(f"{symbols.get('info', 'ℹ️')} Renderd service status:", "info", logger_to_use, app_settings)
    run_elevated_command(["systemctl", "status", "renderd.service", "--no-pager", "-l"], app_settings,
                         current_logger=logger_to_use)
    log_map_server(f"{symbols.get('success', '✅')} Renderd service activated.", "success", logger_to_use, app_settings)