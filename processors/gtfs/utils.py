...
)  # To access GTFS_FILE_SCHEMAS and Pydantic models
...
def get_db_connection(db_params: Dict[str, str] = None) -> Any:
    ...
    try:
        ...
        logger.info(
            f"Connected to database {params.get('dbname')} on {params.get('host')}:{params.get('port')}"
        )
        return conn
...
def validate_dataframe_with_pydantic(
    ...
    for index, row in df.iterrows():
        try:
            ...
            for col, val in row.items():
                ...
                record_dict[col] = (
                    val if pd.notna(val) and str(val).strip() != "" else None
                )
            ...
            )  # exclude_none=False to keep explicit None
            ...
            logger.debug(
                f"Validation failed for record at index {index} from {gtfs_filename}: {e.errors()}"
            )
            ...
            invalid_records_info_list.append(invalid_record_info)
            logger.error(
                f"Unexpected error validating record at index {index} from {gtfs_filename}: {ex}",
                exc_info=True,
            )
    ...
    if valid_records_list:
        ...
        valid_df_columns = (
            list(valid_records_list[0].keys()) if valid_records_list else []
        )
...
if __name__ == "__main__":
    ...
    raw_stop_data = {
        ...
        "stop_name": [
            ...
            "Stop Six (Good)",
        ],  # Test stripping, empty after strip
    ...
    valid_stops_df, invalid_stops_info = validate_dataframe_with_pydantic(
        raw_stops_df, stop_pydantic_model, "stops.txt"
    )
    ...
    if (
            valid_stops_df.empty and not invalid_stops_info
    ):  # Check if something unexpected happened
        logger.warning(
    ...
    valid_agency_df, invalid_agency_info = validate_dataframe_with_pydantic(
        raw_agency_df, agency_pydantic_model, "agency.txt"
    )
    logger.info(
