...
def create_tables_from_schema(conn):
    ...
    for filename in GTFS_LOAD_ORDER:
        ...
        if details.get("composite_pk"):
            quoted_pks = [
                sql.Identifier(pk_col) for pk_col in details["composite_pk"]
            ]
...
def drop_all_gtfs_foreign_keys(conn):
    ...
    for from_table, _, _, _, fk_name in reversed(GTFS_FOREIGN_KEYS):
        ...
        if not cursor.fetchone()[0]:
            logger.debug(
                f"Table {from_table} for FK {fk_name} does not exist. Skipping drop."
            )
            continue
        ...
        if cursor.fetchone():
            try:
                ...
                logger.warning(
                    f"Could not drop foreign key {fk_name} from {from_table}: {e}"
                )
        else:
            logger.debug(
                f"Foreign key {fk_name} on {from_table} does not exist. Skipping drop."
            )
    cursor.close()
...
def add_foreign_keys_from_schema(conn):
    ...
    for (
        ...
        if not cursor.fetchone()[0]:
            logger.warning(
                f"Source Table {from_table} for FK {fk_name} does not exist. Skipping."
            )
            continue
        cursor.execute("SELECT to_regclass(%s);", (f"public.{to_table}",))
        if not cursor.fetchone()[0]:
            logger.warning(
                f"Target Table {to_table} for FK {fk_name} does not exist. Skipping."
            )
            continue
        ...
        try:
            ...
            try:
                dlq_cursor = (
                    conn.cursor()
                )  # Use a separate cursor for DLQ if main one is in error state from alter
                dlq_cursor.execute(
...
def load_data_to_db_with_dlq(conn):
    ...
    for gtfs_filename_key in GTFS_LOAD_ORDER:
        ...
        if not actual_file_to_load:
            if gtfs_filename_key in GTFS_DEFINITIONS:
                logging.warning(
                    f"Expected GTFS file '{gtfs_filename_key}' not found in archive. Skipping."
                )
            continue
        ...
        try:
            ...
            if df.empty:
                logging.info(
                    f"File {actual_file_to_load} is empty. Skipping table {table_name}."
                )
                continue
            ...
            if not actual_load_cols:
                logging.warning(
                    f"No columns to load for {table_name} from {actual_file_to_load}. Skipping."
                )
                continue
            ...
            copy_sql_stmt = sql.SQL(
                "COPY {} ({}) FROM STDIN WITH CSV HEADER DELIMITER ',' QUOTE '\"' ESCAPE '\\' NULL AS '';"
            ...
            if table_name == "gtfs_stops":
                ...
                if (
                    ...
                    try:
                        ...
                        logging.info(
                            f"Geometry updated for {table_name} ({cursor.rowcount} rows affected)."
                        )
            ...
            logging.error(
                f"Database error processing {actual_file_to_load} for {table_name}: {db_err}",
                exc_info=True,
            )
            try:
                ...
                logging.error(
                    f"Failed to write to DLQ for {actual_file_to_load} after DB error: {e_dlq}"
                )
            raise
        except Exception as e:
            logging.error(
                f"Unexpected error processing file {actual_file_to_load} for table {table_name}: {e}",
                exc_info=True,
            )
            try:
                ...
                logging.error(
                    f"Failed to write to DLQ for {actual_file_to_load} after unexpected error: {e_dlq}"
                )
            raise
    ...
    if (
        ...
        try:
            ...
            logging.error(
                f"Error aggregating shape linestrings: {e}", exc_info=True
            )
            try:
                dlq_cursor = conn.cursor()
                dlq_cursor.execute(
                    ...
                    (
                        "shapes.txt",
                        f"Aggregation to lines failed: {e}",
                ...
                logging.error(
                    f"Failed to write to DLQ for shape aggregation error: {e_dlq}"
                )
...
def run_full_gtfs_etl_pipeline():
    ...
    try:
        if (
            GTFS_URL == "https://example.com/path/to/your/gtfs-feed.zip"
            and "GTFS_FEED_URL" not in os.environ
        ):
        ...
        if not download_and_extract_gtfs():
            raise Exception(
                "GTFS feed download or extraction failed. See logs for details."
            )
        ...
        if os.path.exists(DOWNLOAD_PATH):
            try:
                ...
                logger.error(
                    f"Error removing download file '{DOWNLOAD_PATH}': {e}"
                )
        if os.path.exists(EXTRACT_PATH):
            try:
                shutil.rmtree(EXTRACT_PATH)
                ...
                logger.error(
                    f"Error removing extract directory '{EXTRACT_PATH}': {e}"
                )
        ...
        if success:
            ...
            logger.error(
                f"GTFS Full Update Process FAILED at {end_time.isoformat()}. Duration: {duration}"
            )
...
def setup_logging(
    log_level=logging.INFO, log_file=LOG_FILE, log_to_console=True
):
    """
...
def main():
    ...
    setup_logging(
        log_level=log_level, log_file=args.log_file, log_to_console=True
    )

    # Get GTFS feed URL from command-line argument or environment variable
    gtfs_url_to_check = args.gtfs_url
    if (
        gtfs_url_to_check == "https://example.com/path/to/your/gtfs-feed.zip"
        and "GTFS_FEED_URL" in os.environ
    ):
    ...
    try:
        ...
        logger.critical(
            f"An error occurred during the GTFS update process: {e}",
            exc_info=True,
        )
        sys.exit(1)
