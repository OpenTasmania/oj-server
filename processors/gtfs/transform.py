...
def create_point_wkt(lon: Any, lat: Any, srid: int = 4326) -> Optional[str]:
    ...
    try:
        ...
        if lon is None or lat is None or str(lon).strip() == "" or str(lat).strip() == "":
            logger.debug(f"Missing lat or lon for WKT creation: lat='{lat}', lon='{lon}'")
            return None
        ...
        if not (-90 <= lat_float <= 90 and -180 <= lon_float <= 180):
            logger.warning(f"Invalid lat/lon values for WKT: lat={lat_float}, lon={lon_float}")
            return None

        return f"SRID={srid};POINT({lon_float} {lat_float})"
    except (ValueError, TypeError) as e:
        logger.warning(f"Could not create POINT WKT due to invalid lat/lon: lat='{lat}', lon='{lon}'. Error: {e}")
        return None


def transform_dataframe(df: pd.DataFrame, file_schema_info: Dict[str, Any]) -> pd.DataFrame:
    """
    ...
    for col in transformed_df.columns:
        if col in file_schema_info.get("columns", {}) and transformed_df[col].dtype == 'object':
    ...
    if geom_config:
        ...
        if lat_col in transformed_df.columns and lon_col in transformed_df.columns and geom_col:
            logger.debug(f"Creating WKT for geometry column '{geom_col}' from '{lat_col}' and '{lon_col}'.")
            transformed_df[geom_col] = transformed_df.apply(
                lambda row: create_point_wkt(row.get(lon_col), row.get(lat_col), srid), axis=1
            )
            if geom_col not in expected_cols:  # Add geom_col to expected if generated
            ...
            if geom_col and geom_col not in transformed_df.columns:  # Ensure geom col exists if expected, even if null
...
def transform_shape_points_to_lines_df(shapes_points_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    if shapes_points_df.empty:
        logger.info("Shapes points DataFrame is empty. No lines to transform.")
        return pd.DataFrame(columns=['shape_id', 'geom'])

    required_cols = ['shape_id', 'shape_pt_lon', 'shape_pt_lat', 'shape_pt_sequence']
    if not all(col in shapes_points_df.columns for col in required_cols):
        logger.error(f"Shapes points DataFrame is missing one or more required columns: {required_cols}")
        return pd.DataFrame(columns=['shape_id', 'geom'])

    logger.info(f"Aggregating {len(shapes_points_df)} shape points into LineStrings...")

    # Ensure correct data types for sorting and geometry creation
    try:
        shapes_points_df['shape_pt_lon'] = pd.to_numeric(shapes_points_df['shape_pt_lon'], errors='coerce')
        shapes_points_df['shape_pt_lat'] = pd.to_numeric(shapes_points_df['shape_pt_lat'], errors='coerce')
        shapes_points_df['shape_pt_sequence'] = pd.to_numeric(shapes_points_df['shape_pt_sequence'], errors='coerce')
    ...
    shapes_points_df.dropna(subset=['shape_id', 'shape_pt_lon', 'shape_pt_lat', 'shape_pt_sequence'], inplace=True)

    # Sort by shape_id and then by sequence
    sorted_shapes = shapes_points_df.sort_values(by=['shape_id', 'shape_pt_sequence'])

    lines = []
    for shape_id, group in sorted_shapes.groupby('shape_id'):
        if len(group) < 2:  # Need at least two points to make a line
            logger.debug(f"Shape_id '{shape_id}' has fewer than 2 valid points. Skipping LineString creation.")
            continue

        # Create coordinate pairs (lon, lat)
        coords = [f"{row.shape_pt_lon} {row.shape_pt_lat}" for _, row in group.iterrows()]
...
if __name__ == "__main__":
    ...
    raw_stop_data = {
        ...
        'stop_name': [' Stop One ', 'Stop Two', 'Stop Three (No Coords)', 'Stop Four (Bad Coords)', 'Stop Five '],
        'stop_lat': ['40.7128 ', '40.7321', None, '95.0', ' '],  # Note spaces, None, bad value, empty string
    ...
    logger.info(f"Transformed stops DataFrame columns: {transformed_stops_df.columns.tolist()}")
    ...
    mock_shapes_points_data = {
        ...
        'shape_pt_sequence': [1, 2, 3, 1, 1, 1],  # shp2_bad has only one point effectively after bad lat
