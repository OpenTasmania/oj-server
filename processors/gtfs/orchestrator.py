...
class GTFSConfigError(Exception): pass
...
def process_and_setup_gtfs(
        ...
        orchestrator_logger: Optional[logging.Logger] = None # Logger for this orchestrator's messages
) -> None:
    """
    ...
    log_map_server(f"{symbols.get('rocket','🚀')} Starting full GTFS setup and processing...", "info", effective_logger, app_settings)

    # Validate essential parameters from app_settings
    if not app_settings.gtfs_feed_url:
        log_map_server(f"{symbols.get('error','❌')} GTFS Feed URL is missing in configuration. Cannot proceed.", "error", effective_logger, app_settings)
    ...
    try:
        ...
        log_map_server(f"{symbols.get('step','➡️')} Step 1: Setting up GTFS environment (logging & OS vars)...", "info", effective_logger, app_settings)
        ...
        log_map_server(f"{symbols.get('success','✅')} GTFS environment setup complete.", "success", effective_logger, app_settings)

        # Step 2: Run the ETL pipeline and verify
        # run_gtfs_etl_pipeline_and_verify now takes app_settings
        log_map_server(f"{symbols.get('step','➡️')} Step 2: Running GTFS ETL pipeline and verification...", "info", effective_logger, app_settings)
        run_gtfs_etl_pipeline_and_verify(
            app_settings=app_settings,
            current_logger=effective_logger
        )
        log_map_server(f"{symbols.get('success','✅')} GTFS ETL pipeline and verification complete.", "success", effective_logger, app_settings)

        # Step 3: Configure cron job for automation
        # configure_gtfs_update_cronjob now takes app_settings
        log_map_server(f"{symbols.get('step','➡️')} Step 3: Configuring GTFS update cron job...", "info", effective_logger, app_settings)
        configure_gtfs_update_cronjob(
            app_settings=app_settings,
            current_logger=effective_logger
        )
        log_map_server(f"{symbols.get('success','✅')} GTFS update cron job configuration complete.", "success", effective_logger, app_settings)

        log_map_server(f"{symbols.get('sparkles','✨')} Full GTFS setup and processing completed successfully.", "success", effective_logger, app_settings)

    except GTFSConfigError as e:
        log_map_server(f"{symbols.get('critical','🔥')} GTFS Configuration Error: {e}", "critical", effective_logger, app_settings)
        raise
    except RuntimeError as e:
        log_map_server(f"{symbols.get('critical','🔥')} GTFS Processing Runtime Error: {e}", "critical", effective_logger, app_settings)
        raise
    except Exception as e:
        log_map_server(f"{symbols.get('critical','🔥')} An unexpected error occurred during GTFS setup: {e}", "critical", effective_logger, app_settings, exc_info=True)
        raise