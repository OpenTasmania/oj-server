...
def setup_logging(
    ...
    if log_file:
        try:
            ...
            logging.error(
                f"Failed to configure file logging to '{log_file}': {e}. Logging to console only.",
                exc_info=True,
            )
            if (
                    not log_to_console
            ):  # Ensure there's at least console logging if file fails and console was off
...
def get_db_connection(
    ...
    if (
        ...
        logging.critical(
            "CRITICAL: Default placeholder password is being used for database connection."
        )
        logging.critical(
            "Please configure a strong password in DB_PARAMS or via PG_OSM_PASSWORD environment variable."
        )
...
def cleanup_directory(
        dir_path: Union[str, Path], ensure_dir_exists: bool = False
) -> None:
    """
        ...
        if dir_path.is_dir():
            for item in dir_path.iterdir():
                try:
                    ...
                    logger.error(
                        f"Error deleting item {item} in {dir_path}: {e}"
                    )
            logger.info(f"Cleaned contents of directory: {dir_path}")
        else:
            logger.warning(
                f"Path {dir_path} exists but is not a directory. Cannot clean."
            )
    else:
        logger.info(
            f"Directory {dir_path} does not exist. No cleanup needed there."
        )
...
if __name__ == "__main__":
    ...
    )  # Set to DEBUG to see all test messages
    ...
    if (
        ...
        logging.warning(
            "Skipping database connection test as password is still the placeholder."
        )
        logging.warning(
            "To test DB connection, set PG_OSM_PASSWORD env var or update DEFAULT_DB_PARAMS."
        )
    else:
        ...
        if test_conn:
            ...
            try:
                with test_conn.cursor() as cur:
                    ...
                    logging.info(
                        f"PostgreSQL version: {pg_version['version']}"
                    )
            except Exception as e:
    ...
    cleanup_directory(
        TEST_CLEANUP_DIR, ensure_dir_exists=False
    )  # Clean contents, don't ensure it exists after (it will be empty)
    remaining_items = [
    ...
    if not any(
            item.startswith("file") for item in remaining_items
    ...
    if TEST_CLEANUP_DIR.exists() and TEST_CLEANUP_DIR.is_dir():
        logging.info(
            f"Cleanup with ensure_dir_exists=True successful. Directory {TEST_CLEANUP_DIR} exists."
        )
    else:
        logging.error(
            f"Cleanup with ensure_dir_exists=True FAILED. Directory {TEST_CLEANUP_DIR} does not exist."
        )
