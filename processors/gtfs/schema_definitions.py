...
class GTFSBaseModel(BaseModel):
    class Config:
        ...
        anystr_strip_whitespace = (
            True  # Strip whitespace from all string fields
        )
        validate_assignment = (
            True  # Validate when setting attributes after creation
        )
...
class Agency(GTFSBaseModel):
    agency_id: Optional[Annotated[str, StringConstraints(min_length=1)]] = (
        None  # Conditionally Required: required if multiple agencies
    )
    agency_name: Annotated[str, StringConstraints(min_length=1)]
    agency_url: Annotated[
        str, StringConstraints(pattern=r"^https?://.+")
    ]  # Basic URL validation
    agency_timezone: Annotated[
        str, StringConstraints(min_length=1)
    ]  # Should be a valid TZ database name
...
class Stop(GTFSBaseModel):
    ...
    stop_name: Optional[Annotated[str, StringConstraints(min_length=1)]] = (
        None  # Conditionally Required if not a station with name
    )
    ...
    location_type: Optional[conint(ge=0, le=4)] = Field(
        0
    )  # 0 for stop/platform, 1 for station, etc. Default to 0 (stop/platform)
    parent_station: Optional[str] = (
        None  # Should be a stop_id of a station (location_type=1)
    )
    stop_timezone: Optional[str] = (
        None  # Optional, if different from agency_timezone
    )
...
class Route(GTFSBaseModel):
    ...
    agency_id: Optional[str] = (
        None  # Conditionally Required if multiple agencies
    )
    route_short_name: Optional[str] = Field(
        "", max_length=50
    )  # GTFS says default empty string is okay
    route_long_name: Optional[str] = Field("", max_length=255)
    route_desc: Optional[str] = None
    route_type: conint(
        ge=0, le=7
    )  # Or more specific if only certain types supported, e.g. 3 for Bus
    ...
    continuous_pickup: Optional[conint(ge=0, le=3)] = Field(
        None
    )  # 0-regular, 1-none, 2-phone, 3-driver
...
class Trip(GTFSBaseModel):
    ...
    wheelchair_accessible: Optional[Literal[0, 1, 2]] = Field(
        None
    )  # 0-no info, 1-accessible, 2-not accessible
    bikes_allowed: Optional[Literal[0, 1, 2]] = Field(
        None
    )  # 0-no info, 1-allowed, 2-not allowed
...
class StopTime(GTFSBaseModel):
    ...
    stop_id: Annotated[str, StringConstraints(min_length=1)]
    stop_sequence: conint(
        ge=0
    )  # Must be non-negative, usually positive and increasing
    stop_headsign: Optional[str] = None
    pickup_type: Optional[conint(ge=0, le=3)] = Field(
        None
    )  # Default is 0 if not provided
    drop_off_type: Optional[conint(ge=0, le=3)] = Field(
        None
    )  # Default is 0 if not provided
    continuous_pickup: Optional[conint(ge=0, le=3)] = Field(None)
    continuous_drop_off: Optional[conint(ge=0, le=3)] = Field(None)
    shape_dist_traveled: Optional[confloat(ge=0)] = None
    timepoint: Optional[Literal[0, 1]] = Field(
        None
    )  # 0-approximate, 1-exact. Default is 1 if not provided.
    ...
    def check_times_conditionally_required(cls, v, values, field):
        ...
        if (
                ...
                and v is None
                and values.get("departure_time") is None
        ):
...
class Calendar(GTFSBaseModel):
    ...
    start_date: Annotated[
        str, StringConstraints(pattern=r"^[0-9]{8}$")
    ]  # YYYYMMDD
    end_date: Annotated[
        str, StringConstraints(pattern=r"^[0-9]{8}$")
    ]  # YYYYMMDD
...
class Frequency(GTFSBaseModel):
    ...
    exact_times: Optional[Literal[0, 1]] = Field(
        None
    )  # 0-frequency based, 1-schedule based
...
class Transfer(GTFSBaseModel):
    ...
    transfer_type: conint(
        ge=0, le=3
    )  # 0-recommended, 1-timed, 2-min_time, 3-not possible
...
class FeedInfo(GTFSBaseModel):
    ...
    def check_date_format_optional(cls, v):
        if (
                v is not None and v != ""
        ):  # Allow empty string if field is truly optional
            try:
                datetime.strptime(v, "%Y%m%d").date()
            except ValueError as e:
                raise ValueError(f"Date {v} is not a valid YYYYMMDD date.") from e
...
if __name__ == "__main__":
    ...
    logger.info(
        "--- Schema definition tests finished (some intentionally commented out to avoid script exit) ---"
    )
